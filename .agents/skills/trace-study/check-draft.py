...
OFF_NOTE = [
    ...
    (r'(고쳐야 한다|고쳐야 합니다|개선하면|넣어야 한다|바꿔야 한다|리팩터)',
     '판정·처방 — 백로그로'),
    # 설계 문서 대조 이력 (2026-09-01 사용자 지적 — "내가 그것까지 알아야 하는거야?
    #  어쩌피 나는 너가 정리하고 방향성을 잡아준 상태에서 이것을 진행하는데?")
    # 「왜 그렇게 정했나」는 쓰되 출처 문서를 인용하지 않는다. 이유만 기능 언어로 쓴다.
    # 잡는 것은 「대조 결과」뿐이다. 설계 문서를 **근거로 인용하는 것**은 (b-1)이 요구하는
    # 정당한 사용이라 잡지 않는다 (2026-09-01 첫 패턴이 노트 21곳을 오탐했다).
    (r'((문서|스펙|설계|킥오프)[^.。]{0,30}(와|과)\s*(지금\s*)?코드'
     r'|코드[^.。]{0,30}(와|과)\s*(문서|스펙)'
     r'|문서 기준으로|문서가 틀린)',
     '설계 문서 대조 이력 — agent-log로. 노트에는 지금 코드가 무엇을 하는지만'),
]
