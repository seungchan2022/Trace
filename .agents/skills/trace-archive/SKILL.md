...
## 4.5 백로그 완료 항목을 아카이브로 옮긴다

이번 MVP에서 해결한 백로그 항목은 `- [x]` + `*resolved(날짜, 어디서):*`로 바꾼 뒤
**`docs/backlog.md`에 두지 말고 [`docs/backlog-archive.md`](../../../docs/backlog-archive.md)로
옮긴다**(2026-09-02 규칙 — 그전에는 본문에 쌓아 두어서 파일의 3분의 1이 끝난 항목이었다).

```bash
grep -c '^- \[x\]' docs/backlog.md   # 0이 아니면 아직 옮길 것이 남아 있다
```

- **절 머리(`## …`)를 함께 옮긴다.** 그 절에 미완료가 남아 있으면 **절 머리를 양쪽에 둔다** —
  아카이브 쪽에도 *어느 QA 회차에서 나왔나*가 있어야 나중에 찾을 수 있다.
- 🔴 **내용을 줄이거나 요약하지 않는다.** 원래의 *what · why deferred · trigger*를 그대로 옮긴다.
  남기는 이유가 결론이 아니라 **예상이 어디서 빗나갔나**여서다(`docs/backlog.md` 머리말).
- **아카이브 머리의 인덱스 표에 건수를 반영한다.** 그 표가 나중에 절을 좁히는 유일한 수단이다.
