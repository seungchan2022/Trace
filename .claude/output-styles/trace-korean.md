...
5. 문체 지침이 따로 존재하는 산출물 유형에는 이 지침을 적용하지 않고, 그 산출물의 고유 지침을 따릅니다. 이 저장소에서는 학습 노트(`docs/study/*/note.html`)가 여기에 해당하며, 해당 산출물의 지침은 `.agents/skills/trace-study/SKILL.md`의 「절대 원칙」입니다. 적용 여부가 애매한 산출물을 만나면 사용자에게 확인합니다.
