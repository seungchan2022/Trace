...
<!-- 원본: https://github.com/snflkd/fluent-korean (MIT, Copyright (c) snflkd). Trace용으로 수정함. 근거는 docs/agent-rules/project-decisions.md -->
...
5. 문체 지침이 따로 존재하는 산출물 유형에는 이 지침을 적용하지 않고, 그 산출물의 고유 지침을 따릅니다. 이 저장소에서는 학습 노트(`docs/study/*/note.html`)가 여기에 해당하며, 해당 산출물의 지침은 `.claude/skills/trace-study/SKILL.md`의 「절대 원칙」입니다. 적용 여부가 애매한 산출물을 만나면 사용자에게 확인합니다.
...
2. 구체적인 의미를 담고 있는 어휘와 자연스러운 통사 구조를 결합하면, 풍부하고 명확한 의미를 전달할 수 있습니다. 따라서 맥락에 적합한 어휘를 선택하고, 그 어휘에 조사와 어미를 붙여서 어휘 사이의 관계를 확실하게 나타내야 합니다. [<쓴 비용을 구하는 토큰 카운트 함수에 문제가 생기면 (상황에 적합한 어휘가 사용되지 않아 의미가 불충분함) /지출 비용 추론 용도의 토큰 카운트 함수의 오류 상황에서 (조사와 어미가 없어 가독성이 낮고 의미 관계가 불분명함)>  → 지출한 비용을 추론하는 토큰 카운트 함수에 오류가 발생하면 (이 지침의 목표 예시)]

3. 한국어 사전에 있는 어휘이며 뜻이 명확하더라도, 사용 빈도가 낮아서 통용되지 않는 어휘를 사용하면 소통의 효율성이 오히려 낮아지므로 자제합니다. 대신 의미가 명확하며 실제로 통용되는 어휘를 우선적으로 선택합니다. 어려운 어휘로 정확도를 올리는 것보다, 통용되는 어휘로 의미 관계를 분명히 밝히는 것이 이 지침의 목표에 부합합니다.

4. 일반적인 어휘를 사용해야 하는 자리에 비유적 어휘를 사용하면 가독성이 낮고, 의미가 변질되기 쉽습니다. 따라서 꼭 필요한 경우가 아니라면 비유적 어휘로 일반적인 명사나 동사를 대체하지 않습니다. 다만 일상적인 문어에서 통용되고 지금 다루는 분야에서도 관용 표현으로 정착되어 있어서, 일반적인 어휘로 바꾸면 오히려 어색해지는 표현은 그대로 사용합니다. [<분석의 흐름 → 분석의 방향성>, <코드로 박는 자리 → 코드에 명시하는 상황 (혹은 코드에 명시하는 작업)>, <요청을 받습니다 -> 요청을 확인했습니다 (혹은 요청대로 수행하겠습니다)>]
