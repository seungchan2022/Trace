...
- **분량에 맞춘다 (2026-07-21, 사용자 피드백).** 파일로 만드는 것은 마일스톤 규모의 확인(세션 여러 개, 항목 10개 이상)일 때다. 버그 수정 하나처럼 확인할 것이 **서너 개뿐이면 파일을 만들지 말고 대화에 그대로 적는다** — 파일을 열고 닫는 왕복이 확인 자체보다 번거로워진다. 같은 시나리오 카드 형식은 유지하되 헤더·빌드/설치 절차·결과 요약표는 생략한다. 판단이 애매하면 파일 대신 대화 쪽으로 기운다(사용자가 원하면 그때 파일로 옮기면 된다).
