...
2. **사이클 루프** — roadmap의 마일스톤을 사이클로 묶어 하나씩 (묶음·종류는 킥오프에서 확정).
   **어느 단계를 밟을지는 위 "작업 종류" 표를 따른다** — 아래 목록은 새 기능·작은 기능 기준이고,
   ...
   - `superpowers:writing-plans` → `docs/superpowers/plans/<날짜>-<슬러그>.md`
     ...
     갱신 같은 종료 절차는 아래 "마일스톤 종료 절차"가 담당한다. 단 **그 절차의 대상 지정
     체크박스는 Task가 아니라 계획 끝의 「종료」 절에 둔다** — 아래 항목 참고.
     — **검증 명령을 계획 문서에 복사하지 않는다.** `testing.md`의 Baseline을 가리킨다. 복사본은
     시뮬레이터 UDID가 환경마다 달라 낡고, `testing.md` 규칙이 바뀌어도 그대로 남는다
     (`lint-cleanup`이 18줄을 복사했다). 그 마일스톤에만 쓰는 명령이 있으면 그것만 적는다.
   - `superpowers:executing-plans` → 구현·리뷰·검증·커밋 (feature 브랜치 1개)
   - **마일스톤 종료 절차 — 어느 경로로 왔든 면제되지 않는다.** 새 기능·작은 기능·정비·버그 어느
     종류든, MVP 안의 마일스톤이든 MVP 밖 독립 사이클이든 이 절차를 그대로 밟는다. 아래 "정비 작업
     경로"와 "버그 처리 경로"는 **착수 쪽 단계만** 줄이는 것이지 종료를 줄이지 않는다.
     **절차 본문은 계획 문서에 복사하지 않되, 계획 끝에 「종료」 절을 두고 대상 지정 체크박스
     2~3줄을 적는다.** 본문 복사를 막는 이유는 MVP17 `sheet-drag`가
     이 절차를 Task 3·4로 넣어 162줄을 차지했고 체크리스트 본문이 `docs/qa/` 파일과 중복됐기
     때문이다. 🔴 **그렇다고 계획에서 통째로 지우면 실행 지점이 사라진다** — 2026-09-03에 실측된
     실패이고, 같은 종류의 사이클이 이틀 전에는 체크박스를 둬서 성공했다
     (`docs/workflow-audit.md` §5-9). 체크박스에 적는 것은 *어느 파일의 무엇을 갱신하는가*이지
     절차 본문이 아니며, 커밋되므로 `git log`로 확인되고 `trace-init`이 읽는다:
     ...
     4. 수용된 경우에만 `docs/roadmap.md`를 갱신한다. **MVP 안이면** 그 마일스톤을 `[x]`로
        켜고(사이클에 여러 개가 묶여 있으면 끝난 것부터 개별로 켠다), **MVP 밖 독립 사이클이면**
        「진행 중」 절의 그 항목을 「완료 · MVP 밖 독립 사이클」로 옮기고 결과·검증·예상이 빗나간
        지점을 남긴다. ⚠️ **독립 사이클에는 마일스톤이 없다** — 이 단계를 마일스톤 표현으로만
        적어 두면 그런 사이클에서 가리킬 대상이 없어 통째로 빠진다(2026-09-03 `pace-definition`).
     5. 같은 시점에 `docs/current-mvp.md`를 갱신한다 — MVP 안이면 마일스톤 결과와 다음 사용자
        확인을, 독립 사이클이면 현재 상태와 다음에 착수할 수 있는 것을 적는다. 그 사이클이 닫은
        백로그 항목이 있으면 **같은 시점에** `docs/backlog-archive.md`로 옮기고, QA에서 실행하지
        못한 확인이 있으면 백로그에 다시 올린다(표기 관례는 `docs/backlog.md` 머리말).
        `docs/current-features.md`는 사용자가 할 수 있는 일이 바뀐 흐름만 갱신한다
        (`docs/agent-rules/product-visibility.md`).
     6. `milestone-retro` 실행을 **제안**한다(강제 아님. 상세는 `skills.md`). MVP 밖 독립 사이클은
        전례 셋 모두 회고 없이 닫았으므로 넘어가도 된다.
...
- **단위는 MVP가 아니라 기능**이다. MVP 순서로 돌면 이후 교체·삭제된 화면을 배우게 된다 —
  MVP 문서는 순회 대상이 아니라 **참조 대상**으로 쓴다. 산출물은 `docs/study/<번호>-<슬러그>/`
