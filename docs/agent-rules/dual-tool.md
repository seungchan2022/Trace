...
- **Exception — the final whole-branch review.** Dispatch it with the model set *explicitly* to the highest-capability model; never let it inherit. Per-task reviews keep inheriting the session model, which matches how MVP14–17 were run. This exists because an implementation session on the balanced model would otherwise downgrade that gate with no error and no visible signal. `superpowers:subagent-driven-development` already mandates explicit model selection here, but `/code-review` (`ce-code-review`) spawns reviewers without one and inherits silently — the rule covers both paths.
