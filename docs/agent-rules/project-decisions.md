...
- 바텀시트 드래그-리사이즈 — P2 백로그 항목이었으나 트리거 조건("탭 토글이 답답하다는 피드백")이 실제로 충족돼 지금 구현으로 당김 (결정 2026-07-12, 사용자). 그래버(handle)에만 `DragGesture` 부착 — sheetHeader/시트 전체에 걸면 내부 Button들과 히트테스트가 충돌해 전부 먹통이 되는 회귀가 실기기에서 확인됨(2026-07-12). 연속 높이 추적이 아니라 두 detent(collapsed/expanded) 사이 임계값(40pt) 스냅 방식 — `isBottomSheetExpanded` 단일 소스는 유지, 탭 토글과 공존. `docs/backlog.md`의 해당 항목은 완료 처리.
