...
| 학습 중 발견 (2026-08-30, 따라잡기 청크 4 파트 3) | 2 | 3 |
...
- [x] **발화와 잠금화면이 서로 다른 페이스를 보여주는데 둘 다 「페이스」로 불린다 — 실사용에서 버그로 의심됐다** — *what:* 1km마다 나오는 음성 발화는 **평균 페이스**를 읽고(러닝 전체 기준), 잠금화면은 **현재 페이스**를 보여준다(최근 30초 기준). 재는 구간이 달라 값이 크게 벌어질 수 있는데 잠금화면 라벨은 그냥 *「페이스」*다. **실사용 관찰(2026-08-30 사용자):** *"방금 오디오로 페이스를 이야기 해줬는데 시간이 잠깐 몇초 지나고 잠금화면을 봤는데 페이스가 갑자기 너무 달라져있는거야. 이래서 뭐가 문제인지에 대해서 이야기를 하려고 했거든."* **두 지표가 있다는 사실 자체를 사용자가 몰랐고 오작동으로 읽었다.** / *where:* 발화 `Trace/Application/RunTracking/RunAudioCoach.swift:125`(평균) ↔ 잠금화면 `Trace/Application/RunTracking/RunActivityController.swift:89`(현재) → `TraceWidgets/RunLiveActivityWidget.swift:27,53`의 라벨 `"페이스"`. 두 값이 한자리에 나란히 뜨는 곳은 앱에 없다. / *why deferred:* **동작은 설계대로이고 버그가 아니다.** 고칠 대상은 값이 아니라 *「어느 페이스인지 알 수 없는 표시」*인데, 라벨을 「현재 페이스」로 바꿀지 · 발화를 현재 페이스로 맞출지 · 잠금화면에 둘 다 띄울지가 **제품 결정**이라 스코핑이 먼저다. / *trigger:* 페이스 관련 화면을 손댈 때, 또는 같은 혼란이 한 번 더 관찰되면. 아래 30초 창 항목과 함께 다룬다. `done` — *resolved(2026-09-03, 독립 사이클 `pace-definition`):* 잠금화면 라벨을 「현재 페이스」로 바꾸고 평균 페이스를 나란히 실었으며, 트래킹 화면 보조 행에도 두 값을 함께 뒀다. 발화는 평균 페이스 그대로 유지했다. **백로그가 「제품 결정이라 스코핑이 먼저」라고 본 것은 맞았다** — 세 후보(라벨만 교체 / 발화를 현재 페이스로 / 둘 다 표시) 중 사용자가 「둘 다 표시」를 골랐고, 그 결정이 잠금화면 첫 행에 다섯째 요소를 넣을 수 없다는 것을 드러내 본문을 두 행으로 나누는 후속 작업까지 불렀다. 실기기 QA에서 두 페이스가 잘리지 않고 나란히 읽히는 것을 확인했다.
- [x] **현재 페이스의 30초 창이 적절한지 재본 적이 없다 — 다른 앱 기준 조사가 먼저다** — *what:* `currentPaceWindowSeconds = 30`은 MVP13이 다른 상수 다섯과 한 줄에 정한 잠정값이고(*"전부 구현 중 튜닝 가능"*), **왜 30초인지 근거가 없고 튜닝된 적도 없다.** 사용자가 *"내가 알기로는 5~10초로 알고 있고 30초는 너무 길다는 이야기를 들었던것 같아서"*라고 했다 — 창이 길면 *「지금 속도」*라는 목적에 비해 반응이 느리다. **창 길이를 바꾸면 값이 얼마나 튀는지도 재본 적이 없다.** / *where:* `Trace/Domain/RunTracking/Entity/RunTrack.swift:8`. 근거 문서는 `history/mvp13/2026-07-13-run-tracking.md:29`, 도입 커밋 `10c3d3a`에 설명 없음. / *why deferred:* **조사가 먼저다** — ①다른 러닝 앱이 어떤 창을 쓰는지 ②창을 줄이면 표시가 얼마나 자주 바뀌는지 실측. 근거 없이 숫자만 바꾸면 지금과 같은 상태가 된다. ⚠️ `CLLocation.speed`는 Apple 문서가 *"actual speed can change many times between the delivery of location events, use this property for informational purposes only"*라고 못박은 값이라, 창을 줄일수록 그 흔들림이 그대로 화면에 온다. / *trigger:* 위 항목과 함께. `done` — *resolved(2026-09-03, 독립 사이클 `pace-definition`):* 창을 10초로 줄였다. **다만 why deferred가 요구한 조사 둘 중 ①만 하고 ②는 하지 못했다.** 업계 관행(Garmin 약 5초, Strava 등)은 설계 문서 「조사 근거」에 정리했으나, **창을 줄이면 표시가 얼마나 자주 바뀌는지는 재지 못했다** — 실기기 QA를 등속 GPX 재생으로 진행했는데 그 방식은 위치만 주고 속도를 주지 않아 현재 페이스가 재생 내내 빈 값이었다. 5초를 배제한 근거도 실측이 아니라 샘플 간격 산술이다(뛰는 페이스에서 5초 창에 샘플 두세 개). 남은 실측은 「10초로 줄인 현재 페이스 창의 표시 안정성을 실제 달리기로 확인한 적이 없다」로 백로그에 다시 올렸다.
