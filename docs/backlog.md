...
- [ ] **가로모드에서 코스 탭 시트를 펼치면 레이아웃이 무너짐(탭바/시트가 화면 밖으로 밀림)** — *what:* 실기기 QA 중 발견(2026-07-20) — 기기를 가로로 돌린 뒤 시트를 medium/full로 올리면 탭바가 화면 아래로 밀려나가고(모든 경우 재현), 특정 조건에서 시트가 위로 화면 밖까지 뚫려 보인다. *원인(실측 완료, opus 조사):* `CoursePlannerPage`의 ZStack(코스 화면 전체)이 가로에서 시트가 펼쳐지면 `RootView`의 `VStack{ ZStack(course); TraceTabBar }`가 배정한 높이보다 더 큰 높이(예: 335→396)를 보고하고, VStack이 이 초과분을 상하 중앙 정렬로 오버플로시켜 코스/지도는 위로, 탭바는 아래로 동시에 밀려난다 — 두 증상이 하나의 원인의 양면. 처음 의심했던 "`topSafeAreaInset`이 회전 후에도 세로 값에 눌러앉는 ratchet 문제"는 실제로 있었지만(실측: 62로 고정, 진짜 가로 값은 0) 이건 원인이 아니라 오히려 현재 시트를 짧게 눌러 위쪽 침범을 막아주는 보호막이었다 — **ratchet만 먼저 고치면 시트가 위로 더 심하게 뚫리는 것도 실측 확인됨(순서 중요: 구조적 오버플로 차단이 반드시 먼저, ratchet 수정은 그다음).** ZStack을 부풀리는 1차 트리거는 `mapView.ignoresSafeArea()`의 높이가 형제(시트)와 무관하게 부모 ZStack 크기로 역전파되는 것으로 추정(미확정 가설, 시트 자체 크기보다 ZStack이 더 크게 관측됨). 세로 모드는 오염 없음(회전 왕복 후에도 클린). 전체 실측값·수정 순서·후보안 3가지: `.git/sdd/task-landscape-layout-report.md`(세션 종료 시 사라질 수 있으니 재현 전 우선 원인 분석 재확인 필요 — 리포트 핵심 내용은 이 항목에 요약돼 있음). *why deferred:* 원인 분석은 끝났지만 과거 이 자리(다이내믹 아일랜드 침범 버그)에서 순서를 잘못 잡아 실제 회귀를 낸 이력이 있어, 신중한 순차 구현(구조적 클램프 → 검증 → ratchet 수정 → 재검증)이 필요하고 사이클 1이 이미 길어져 지금 하지 않기로 함(2026-07-20, 사용자 판단). 세로 사용에는 영향 없음(핵심 경험 아님). *update(2026-07-20):* 플랜 작성 완료 — `docs/superpowers/plans/2026-07-20-landscape-sheet-overflow.md` (3중 방어: ① RootView GeometryReader 구조 클램프 → ② 시트 예산 pageHeight min-클램프 → ③ ratchet size class별 분리, 순서 필수 + 6조합 검증 + 실기기 체크리스트 포함). 조사 리포트(`.git/sdd/task-landscape-layout-report.md`)의 실측값·원인 분석은 플랜 §배경에 전문 흡수돼 리포트가 사라져도 무방. 다음 세션에서 `feature/landscape-sheet-overflow` 브랜치로 SDD 실행. *resolved(2026-07-20):* 3중 방어로 수정 — ① RootView GeometryReader 구조 클램프 ② 시트 예산 pageHeight min-클램프 ③ SafeAreaInsetLatch(size class별 ratchet). 6조합 실측 통과. 플랜: `docs/superpowers/plans/2026-07-20-landscape-sheet-overflow.md`. `done`
