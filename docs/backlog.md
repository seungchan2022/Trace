...
- [ ] **actor isolation 전면 정리 — 암묵적 + 명시적 @MainActor 감사 (Swift 6 준비)** — *where:* ① 암묵: Domain 값 타입(`CourseCoordinate`·`CourseSegment`·`SavedCourse` 등)·`CoursePersistenceDTO`·`CourseRecord`·테스트 파일들 ② 명시: `LocationServiceProtocol`·`CoursePlanningServiceProtocol`(Domain **프로토콜**에 `@MainActor`가 붙어 있어 모든 구현체를 main으로 강제 — 재검토 대상, 2026-07-08 사용자 지적), `CourseEditSession`·`DependencyContainer`·`ContinuationBroadcaster`의 `@MainActor`도 필요성 감사 / *now:* (암묵) 프로젝트가 `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`라 순수 값 타입·DTO까지 암묵적 @MainActor — actor 저장소나 테스트(nonisolated)에서 쓰일 때 "Swift 6 언어 모드에서 에러" 경고 가족 ~40건(2026-07-08 전체 리빌드로 확인, 런타임 동작 문제는 없음). `decodeCourseSegments`에 nonisolated만 붙여봤더니 경고가 DTO 내부로 옮겨갈 뿐이라 원복 — 근원은 타입 단위 isolation 미명시. (명시) UI와 무관한 계층(특히 Domain 프로토콜)에 붙은 @MainActor는 설계 의도 없이 관성으로 붙었을 가능성 / *desired:* 한 마일스톤에서 일괄 감사·정리 — 도메인 값 타입·DTO에 `nonisolated` 명시, Domain 프로토콜의 @MainActor는 "정말 main 전용인가"를 계층별로 판정해 제거 또는 유지 근거 기록(`project-decisions.md`). ViewModel·Page 등 UI 계층은 @MainActor 유지. Swift 6 언어 모드 전환 전 필수. `open`
