...
- [ ] **default MainActor isolation ↔ 도메인·DTO isolation 정리 (Swift 6 준비)** — *where:* Domain 값 타입(`CourseCoordinate`·`CourseSegment`·`SavedCourse` 등)·`CoursePersistenceDTO`·`CourseRecord`·테스트 파일들 / *now:* 프로젝트가 `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`라 순수 값 타입·DTO까지 암묵적 @MainActor — actor 저장소나 테스트(nonisolated)에서 쓰일 때 "Swift 6 언어 모드에서 에러" 경고 가족 ~40건(2026-07-08 전체 리빌드로 확인, 런타임 동작 문제는 없음). `decodeCourseSegments`에 nonisolated를 붙여봤더니 경고가 DTO 내부로 옮겨갈 뿐이라 원복 — 근원은 도메인·DTO 타입의 isolation 미명시 / *desired:* 도메인 값 타입과 DTO에 `nonisolated`(또는 Sendable 정합) 명시로 경고 가족 제거. 기계적이지만 Domain 전반을 건드리므로 별도 마일스톤으로. Swift 6 언어 모드 전환 전 필수. `open`
