...
- [x] **임시 마커 라벨이 확정 직후 짧게 뒤바뀌어 보임** — 이번에 해결: `isFirstPoint`(`pendingTapStart == nil`) 판정이 라우팅 완료 전에 이미 바뀌어 출발/도착 핀 스타일을 순간적으로 오재사용하던 버그. 임시 마커를 중립 스타일("확인 중", 회색 `circle.dashed`)로 통일해 판정 자체를 제거. 커밋 119047c. 재검증 완료(`docs/qa/2026-07-04-gesture-consistency-device-checklist.md` 시나리오 1). `done`
