...
- [ ] **SwiftData ModelContext 스레딩 경고** — *where:* `SwiftDataCourseRepository` / *now:* 실기기에서 코스 저장 시 콘솔에 "SwiftData.ModelContext: Unbinding from the main queue. This context was instantiated on the main queue but is being used off it. ModelContexts are not Sendable, consider using a ModelActor." 경고. 빌드 시에도 같은 파일 14행에 "actor-isolated property 'context' can not be mutated from the main actor; this is an error in the Swift 6 language mode" 경고가 있어 서로 일관됨 — main에서 만든 컨텍스트를 백그라운드에서 사용 중. 크래시·데이터 손실은 관찰 안 됨(2026-07-08 QA) / *desired:* `@ModelActor`로 감싸거나 컨텍스트 사용 스레드를 일원화. Swift 6 언어 모드 전환 시 컴파일 에러로 승격될 예정이라 그 전에 정리 필요. `open`
