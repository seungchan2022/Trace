...
- [ ] **SwiftData ModelContext 생성 위치(affinity) 경고** — *where:* `SwiftDataCourseRepository` / *now:* 실기기에서 코스 저장 시 콘솔에 "SwiftData.ModelContext: Unbinding from the main queue. This context was instantiated on the main queue but is being used off it. ModelContexts are not Sendable, consider using a ModelActor." 경고. 빌드 시에도 같은 파일 14행에 "actor-isolated property 'context' can not be mutated from the main actor; this is an error in the Swift 6 language mode" 경고가 있어 서로 일관됨. **동시성 버그는 아님** — 저장소가 이미 `actor`라 모든 접근이 직렬화되고, 문제는 `init`(main 스레드에서 호출)에서 생성한 컨텍스트를 actor 실행기에서 쓰는 생성 위치 위반뿐(2026-07-08 코드 확인). 크래시·데이터 손실 관찰 안 됨(2026-07-08 QA) / *desired:* `@ModelActor` 적용으로 컨텍스트를 actor 실행기에서 생성 — 어댑터 파일 내부 국한, 프로토콜·도메인 무변경, 기존 테스트로 검증 가능한 소규모 수정. Swift 6 언어 모드 전환 시 에러로 승격 예정. **타이밍: 다음 persistence 확장 MVP(달리기 기록 등) 착수 시 첫 정지작업으로** — 새 저장소를 만들기 전에 고쳐야 결함 패턴이 복제되지 않는다. `open`
