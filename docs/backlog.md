...
- [ ] **되짚어 오기 (마지막 구간 역방향 붙이기)** — *what:* 구간 패널 마지막 구간에 "되짚어 오기" 액션 추가 — 역방향 하나만 붙여 코스 끝을 그 구간 시작점으로 되돌림(그리던 중 막다른 골목·방파제에서 돌아 나와 이어 그리기 용도). MVP11 `roundtrip-insert`(왕복 추가)와 같은 기반(좌표 뒤집기 + 패널 액션) 위에 버튼 하나 얹는 소규모 / *trigger:* 왕복 추가만으로는 "그리던 중 되돌아 나오기" 흐름이 불편하다고 실사용에서 확인되면. `open`
