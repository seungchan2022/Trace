...
- [ ] **run-fullscreen** — 러닝 탭 나이키식 전체화면(러닝 탭 전체 지도 제거, 포인트 버튼 유지) + 기록 목록/상세 전체화면 페이지 정리 + 포인트 구간 폴리라인 표시. *아이디어 추가(2026-07-20, tab-restructure QA 중 제안):* 러닝 시작 카운트다운(3-2-1)을 별도 전체화면 페이지로 열어, 지금처럼 카운트다운 중 다른 탭으로 잠깐 이동할 수 있는 상태 자체를 원천 차단(나이키 앱 방식). 지금은 트래킹 시작 시 러닝 탭으로 강제 전환하는 방식으로 갇힘 버그만 막아둔 상태(`feature/mvp16-tab-restructure` 커밋 c7d55fe) — 이 아이디어는 그 위에 얹는 설계 개선이라 버그는 아님.
