...
### 예정 · MVP 밖 독립 사이클 `pace-dedup` (정비)

> 같은 축에서 나온 나머지 세 건(백로그 ①②③) — 평균 페이스 계산식이 네 곳에 복사된 것과
> 앱·위젯 포맷터 이중 구현을 정리한다. 60분/km 상한의 근거 기록은 포맷터를 손보는 김에 함께
> 남긴다(같은 줄이다).
>
> 모아 올 **목적지는 `pace-definition`이 이미 만들어 뒀다** — `RunSession`의 활동 시간 기준 평균
> 페이스 계산 프로퍼티다. 그래서 이 사이클의 범위는 「네 곳을 그 자리로 옮긴다」로 줄어든다.
>
> 새 동작이 없어 설계 문서 없이 **대상 목록·분류 표**로 시작한다(`workflow.md` 정비 작업 경로).
>
> **착수 조건이 풀렸다(2026-09-03)** — `pace-definition`이 완료돼 목적지가 생겼으므로 지금 시작할 수 있다.
