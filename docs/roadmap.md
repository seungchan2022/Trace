...
### MVP8 — 코스 편집·가독성 UX 마무리   (상태: 진행 중)

> 지도 위 경로 표현과 편집 조작을 마무리한다: 구간 패널 스크롤 제한, 핀치 줌 네이티브 복원,
> 겹치는 경로 좌표 오프셋 렌더링. 이후 기능(코스 저장 등)이 독립적으로 붙을 기반.

- [ ] **course-ux-polish** — 구간 패널 최대 높이(화면 ~40%) + 초과 시 스크롤 + 최신 구간 자동 스크롤, 그리기 모드 핀치 줌 네이티브 복원(커스텀 pinchGR 삭제)
- [ ] **overlap-offset** — 겹침 구간 감지(점별 거리 비교) + 진행 방향 수직 좌표 오프셋(~4m) 표시 전용 렌더링
