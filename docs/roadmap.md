...
### MVP15 — 러닝 경험 보강   (상태: 사이클 ① 완료(실기기 QA 통과) → 사이클 ② 플랜 작성 완료·구현 대기)
...
- [ ] **run-waypoints** — 포인트 찍기: 트래킹 버튼·즉시 발화, 잠금화면 버튼(무세션 가드), additive 스키마 확장, 기록 상세 표시·삭제 (표준 사이클: + 최종 브랜치 리뷰(opus)). 플랜 작성 완료(2026-07-18, Task 8개: 세션 마킹 → 발화(AnnouncementKind 끼어들기) → 스키마 v4 → 트래킹 UI → 잠금화면 인텐트 → 기록 상세 표·마커 → 개별 삭제 → QA 체크리스트) — 구현은 새 세션에서 subagent-driven-development로. 플랜: `docs/superpowers/plans/2026-07-18-run-waypoints.md`
