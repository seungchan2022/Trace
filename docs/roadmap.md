...
- [ ] **draw-gesture** — 그리기 제스처 개편(한 손가락=이동, 롱프레스-드래그=그리기) + MVP10 탭 판정 충돌 검증. **플랜 작성 완료(2026-07-21), 구현 미착수** — 사용자 지시로 이 세션은 문서까지만, 구현은 별도 세션에서 진행. 플랜: `docs/superpowers/plans/2026-07-21-draw-gesture.md`(Task 4개, 체크박스 전부 미완료 = 실제 미착수) · 브랜치 `feature/mvp16-draw-gesture`(플랜 문서 커밋만 올라가 있음, 코드 변경 없음)
