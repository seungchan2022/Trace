...
### 진행 중 · MVP 밖 독립 사이클 `pace-display` (착수 2026-09-02)

> **아카이빙 대상이 아니다** — MVP가 아니므로 `trace-archive`가 돌지 않는다.
> 계획·설계 문서는 `docs/superpowers/`에 그대로 둔다.
>
> 아래 「다음 착수 순서」 1순위 「페이스 표시·계산」 축(백로그 5건)을 마일스톤 2개로 잡았다.
> 착수 근거는 2026-08-30 실사용 관찰이다 — 1km 발화 직후 잠금화면을 보니 페이스가 크게 달라
> 있어 오작동으로 읽혔다.
>
> - [ ] `pace-definition` (작은 기능) — 잠금화면·트래킹 화면이 어느 페이스를 보여주는지 이름으로
>   드러내고, 현재 페이스 창을 목적에 맞게 줄인다. 백로그 ④⑤
> - [ ] `pace-dedup` (정비) — 평균 페이스 계산이 네 곳에 복사된 것과 앱·위젯 포맷터 이중 구현을
>   정리한다. 백로그 ①②③
>
> ⚠️ **`pace-definition`이 먼저다.** 잠금화면·발화를 어떻게 정하느냐에 따라 `pace-dedup`이
> 정리할 중복 대상이 달라진다. 반대 순서로 하면 한 곳으로 모은 직후 그 자리를 다시 뜯게 된다.
>
...
| 순위 | 축 | 건수 | 단위 제안 | 조건 |
|---|---|---:|---|---|
| 1 | 페이스 표시·계산 | 5 (+딸려올 후보 2) | **MVP 밖 독립 사이클** — 마일스톤 2개 정도라 MVP 기준(2~5개)에 미달한다 | 발동 — 2026-08-30 실사용 관찰(발화와 잠금화면의 페이스가 달라 오작동으로 읽혔다) |
