...
### 다음 착수 순서 (2026-09-02 분석)

[`docs/backlog.md`](backlog.md) 미완료 60건(분석 시점 기준)의 **`trigger` 필드를 전수 대조해** 뽑았다. 묶는 축은
백로그 절 머리(어느 회차에서 나왔나)가 아니라 **항목들이 서로를 참조하는 작업 축**이다.
단위는 제안이고, **범위 확정은 각 킥오프에서 사용자가 한다**(`workflow.md` 작업 단위 절).

| 순위 | 축 | 건수 | 단위 제안 | 조건 |
|---|---|---:|---|---|
| ~~1~~ **5/5 완료** | 페이스 표시·계산 | 5 중 5건 완료(`pace-definition` 2건 · `pace-dedup` 3건) | **MVP 밖 독립 사이클 2개** — 사용자가 새로 할 수 있게 되는 일이 없어 MVP 기준의 「가치 완결」을 충족하지 않는다. 이미 있는 표시의 정정 + 내부 중복 정리다 | 완료 — 2026-08-30 실사용 관찰(발화와 잠금화면의 페이스가 달라 오작동으로 읽혔다)에서 발동, 두 사이클로 해소 |
| 2 | 코스 편집 UX + 지도 렌더링 + `MapViewRepresentable` 분해 | 10 | **MVP 1개** (마일스톤 3~4개) | 발동 — 2026-08-19·21·24 관찰(핀·흰 점 깜빡임, 고른 구간이 안 풀림) |
| 3 | 코스 계획 라우팅 | 7 | **MVP 밖 독립 사이클** — 사용자 가치가 없는 내부 정리다 | 미발동. 다만 다른 축에 걸리지 않아 언제든 착수할 수 있다 |
| — | 대기 | 38 | 순서에 넣지 않는다 | 실사용 조건이 아직 오지 않았다 |

**축 판정 기준**(누구든 `grep`으로 재현·검산할 수 있게 남긴다) — 항목의 `trigger`에
`코스 계획 라우팅`·`코스 계획 에러 안내`·`캐시` → 3순위 / `코스 편집 UX`·`지도 렌더링`·
`MapViewRepresentable`·`선 굵기·케이싱`·`탭 판별` → 2순위 / `페이스`(또는 바로 앞 페이스 항목을
참조) → 1순위 / 나머지는 대기. 「되돌린 뒤 새로 그리면 영영 사라진다」는 **사용자와 이야기하기로
예약된 항목**이라 대기가 아니라 2순위에 넣었다 — 그 전에 따로 열어도 된다고 적혀 있다.

**1·3순위에 MVP 절차를 씌우지 않는다.** 판정 근거는 마일스톤 개수가 아니라 **가치 완결성**이다
(2026-09-02 정정 — 「마일스톤 2개라 2~5 기준에 미달」이라 적었으나 2개는 범위 안이라 근거가
틀렸다). 두 순위 모두 사용자가 새로 할 수 있게 되는 일이 없다. MVP로 잡으면 킥오프(`brainstorming` decompose ·
로드맵 마일스톤 등록 · 킥오프 문서의 마일스톤 표) · 현황판 갱신 · `ce-doc-review` · 완료 회고 ·
`trace-archive`가 따라오는데, 마일스톤 2개짜리 정리에는 과잉이다. 전례가 둘 있다 —
`study-chunk-0-cleanup`(2026-08-04)과 `current-feature-inventory`(2026-09-01). 전자의 착수
기록이 근거를 그대로 적어 두었다: *"억지로 우산을 씌우면 로드맵이 「MVP인 척하는 잡동사니」를
갖게 된다."* 독립 사이클은 아래 「완료 · MVP 밖 독립 사이클」 절에 기록하고 아카이빙하지 않는다.

⚠️ **2순위의 세 갈래는 쪼갤 수 없다.** 「핀과 경로선 전량 재그리기」의 조건이
*"`MapViewRepresentable` 분해 마일스톤과 **함께**"*이며 *"지금 국소 수정하면 그 분해와 충돌한다"*로
적혀 있고, 흰 점 3건은 *"셋 다 `WaypointDotsRenderer`가 언제 다시 그려지는지와 얽혀 있다"*,
분해 자체는 *"쪼개기가 마일스톤 1개 규모"*다(`MapViewRepresentable.swift` 688줄).

**정리 항목에 단독 사이클을 주지 않는다.** 백로그 항목 대부분이 *"그 영역을 어차피 손보는
마일스톤이 생기면 함께"*를 조건으로 달고 있어서, 해당 기능 사이클 안에 넣는 것이 원래 설계다
(2026-09-02 사용자 확인). 워크플로 항목 3건(GitHub PR 흐름 · 아이디어 뽑기 스킬 · plan 체크박스
훅)은 조건이 *"다음 MVP를 한 번 돌려야 발동"*이라 MVP로 잡지 않고 관찰 대상으로 둔다.

## 완료 · MVP 밖 독립 사이클

### 페이스 계산·표기 통합 `pace-dedup` (MVP 밖 · 독립, 작은 기능 — 착수 직후 정비에서 승격)   (상태: ✅ 완료 2026-09-03)

> **아카이빙 대상이 아니다** — MVP가 아니므로 `trace-archive`가 돌지 않는다.
> 설계·계획 문서는 `docs/superpowers/`에 그대로 둔다.
>
> **결과: 평균 페이스 계산식과 페이스 표기 규칙이 각각 한 곳으로 모였다.** `RunSession`이 계산을
> `private` 헬퍼 하나로 갖고, 러닝 중(`averagePaceSecondsPerKm(now:)`)·요약(`summaryAveragePaceSecondsPerKm`)
> 두 진입점이 그 헬퍼를 공유한다. `RunPaceFormatter`는 프로젝트 파일의 멤버십 예외로 위젯
> 타깃과 공유해, "위젯은 앱 타깃 타입을 볼 수 없다"던 전제가 틀렸음을 확인했다(빌드 구성
> 변경이 아니라 한 줄짜리 프로젝트 파일 편집이었다). **유일한 동작 변화:** 위치 권한이 회수돼
> 종료 버튼 없이 끝난 러닝의 요약에서, 시간 기준이 활동 시간으로 바뀌고 평균 페이스가 값을
> 갖게 됐다 — 이전에는 GPS 구간 기준 시간에 평균 페이스가 비어 있었다. 백로그 3건(평균 페이스
> 식 복사 · 60분 상한 근거 · 포맷터 이중 구현)을 닫아 [`backlog-archive.md`](backlog-archive.md)로
> 옮기고, 새로 발견한 7건(저장 경로에 남은 식 두 자리 · `SavedRun.swift`의 낡은 주석 ·
> `RunDurationFormatter` 이중 구현 · `RunSession.streamEnded()`의 `finish()`와의 중복 ·
> `.swiftlint.yml`의 `TraceWidgets` 누락 · 60분 상한 경계의 두 번째 중복 · 도달 불가 GPS-폴백
> 코드)을 백로그에 올렸다.
>
> **진행 중 예상이 두 번 뒤집혔다 — 같은 모양으로.** Task 2, Task 3 각각의 테스트가 계획 문서에
> **그대로 적힌 코드**였는데도, 논리 시간으로 시뮬레이션한 일시정지 구간(60초)과 실제 벽시계
> 실행 시간(밀리초)이 부딪혀 활동 시간이 음수가 되는 결함이 있었다. 둘 다 `RunSession.finish()`에
> 이미 있던 `now:` 주입 관례(`pause(now:)`·`resume(now:)`)를 그대로 확장해(`finish(now:)`)
> 고쳤고, 그때마다 계획의 「구현 중 기록」과 별도 커밋으로 범위·근거·안전성을 밝혔다. 최종
> 브랜치 리뷰가 이 패턴이 **두 번 반복됐다**는 점을 지적했다 — 다음에 같은 모양의 테스트를 쓸
> 때의 재료로 남긴다(*"논리 시간을 한 `now:`에 주입하면, 종료 호출을 포함해 그 테스트의 모든
> `now:`에 주입해야 한다"*).
>
> **최종 브랜치 리뷰(opus)가 Important 2건을 잡았다** — 위젯의 낡은 주석(공유가 이미 됐는데
> "볼 수 없다"고 계속 주장) · 새 테스트 하나의 핵심 단언이 동어반복이라 회귀를 못 잡는 문제.
> Minor 2건(발화 테스트의 페이스 내용 미검증 · Task 4 빌드 증거 미기록)과 묶어 수정 커밋
> 하나로 해소하고 재리뷰로 확인했다.
>
> **실기기 QA는 조건부 면제**(설계 문서 판단 — 계산은 자동 테스트가 덮고, 잠금화면 렌더링은
> 시뮬레이터로 신뢰할 수 없어 실기기가 아니면 애초에 검증되지 않는다). 잠금화면 페이스 표시
> 확인은 사이클 종료를 기다리지 않고 백로그의 실주행 확인 항목으로 올렸다 —
> `pace-definition`이 남긴 「10초로 줄인 현재 페이스 창」 항목 바로 옆이다(`docs/workflow-audit.md`
> §5-11).
>
> 검증: 빌드·테스트·린트 스탬프 3개 통과(Task 4는 앱 스킴 `Trace`와 위젯 스킴
> `TraceWidgetsExtension`을 각각 빌드) · 자동 테스트 385개(단위) + 8개(UI) 전체 통과 ·
> `RunSession.swift` SwiftLint `type_body_length` 308/300(기존 303/300에서 +5, 이 사이클이
> 허용한 범위 안).
>
> 설계: [`2026-09-03-pace-dedup-design.md`](superpowers/specs/2026-09-03-pace-dedup-design.md) ·
> 계획: [`2026-09-03-pace-dedup.md`](superpowers/plans/2026-09-03-pace-dedup.md)

<details>
<summary>착수 시점 기록 (2026-09-03)</summary>

> 위 「다음 착수 순서」 1순위 「페이스 표시·계산」 축의 나머지 세 건(백로그 ①②③)에서
> 시작했다 — `pace-definition`이 만든 목적지(`RunSession`의 활동 시간 기준 평균 페이스 계산
> 프로퍼티)로 계산식 복사본과 앱·위젯 포맷터 이중 구현을 모은다.
>
> 🔴 **착수 직후 정비에서 작은 기능으로 승격했다(2026-09-03, 사용자 승인).** 대상을 실측하니
> 백로그가 네 곳이라 한 계산식이 **일곱 자리**였고, 그중 둘은 `RunSession`이 없는 자리라 모을
> 수 없으며, 요약 경로를 모으려면 세션에 진입점을 새로 만들고 위젯 타깃 구성을 건드려야 했다.
> 정비 조건 중 「범위가 좁고 명확하다」가 깨졌고, 예외 경로 하나에서는 동작도 달라지는 것으로
> 드러났다.
>
> ⚠️ 이 승격 과정에서 **정비 경로 자체의 구멍 셋**이 드러나 `docs/workflow-audit.md` §5-10에
> 기록했다(규칙 개정은 다음 브랜치로 미룸 — 한 브랜치는 한 가지 변경이다). §5-9(계획 끝
> 종료 체크박스 처방)의 시험도 이 사이클로 한 바퀴 돌았다 — 결과는 §6-A에 기록했다.

- 작업 종류: **작은 기능** — 설계 문서 ○ · 문서 리뷰 ✕ · 계획 문서 ○ · 실기기 QA 조건부 면제
  (설계 판단) · 최종 브랜치 리뷰 ○(작은 기능 표에서는 ✕이나 실제로 실행 — `pace-definition`과
  같은 패턴이고, 이번에도 Important 2건을 잡았다).

</details>

### 페이스 두 지표 표시 `pace-definition` (MVP 밖 · 독립, 작은 기능)   (상태: ✅ 완료 2026-09-03)

> **아카이빙 대상이 아니다** — MVP가 아니므로 `trace-archive`가 돌지 않는다.
> 설계·계획 문서는 `docs/superpowers/`에 그대로 둔다.
>
> **결과: 트래킹 화면과 잠금화면 모두에서 「현재 페이스」와 「평균 페이스」가 라벨로 구분돼
> 나란히 보인다.** 현재 페이스 창은 30초에서 10초로 줄였고, 평균 페이스는 `RunSession`에 새로
> 만든 활동 시간 기준 계산 지점 하나에서 나온다 — 잠금화면은 값을 새로 계산하지 않고 받아 쓴다.
> 발화는 평균 페이스 그대로 뒀다. 백로그 2건(발화·잠금화면의 페이스 불일치 · 30초 창 미검토)을
> 닫아 [`backlog-archive.md`](backlog-archive.md)로 옮겼다.
>
> **진행 중에 예상이 한 번 뒤집혔다.** 잠금화면 첫 행에 다섯째 요소를 그대로 얹었는데, 최종
> 브랜치 리뷰가 실측 폰트 폭으로 잘림을 지적해 본문을 두 행으로 나눴다. 시뮬레이터의 짧은 상태
> 확인만으로는 드러나지 않는 문제였다. ⚠️ **작업 종류 표는 「작은 기능」에 최종 브랜치 리뷰를
> ✕로 두는데, 이번에는 그 리뷰가 실제 결함을 잡았다** — 다음에 같은 종류를 판정할 때의 재료다.
>
> **실기기 QA 결과.** 잠금화면은 일시정지를 포함한 모든 상태에서 통과했다. Dynamic Island는
> 러닝 중 거리·현재 페이스가 보이지 않는데, `main`과 A/B로 대조해 **이 사이클 이전부터 있던
> 문제로 확정**하고 백로그로 넘겼다. ⚠️ **창 값 실측(QA 항목 5)은 실행하지 못했다** — QA를 등속
> GPX 재생으로 진행한 탓에 현재 페이스가 재생 내내 빈 값이었다. 10초를 고른 근거는 업계 조사와
> 자동 테스트뿐이고, 남은 실측은 백로그에 항목으로 다시 올렸다.
>
> 검증: 빌드·테스트·린트 스탬프 3개 통과 · 자동 테스트 388개 전체 통과
> (마지막 Swift 커밋 `98d1b5a` 기준. 이후 커밋은 문서 전용이라 빌드에 닿지 않는다).
>
> 설계: [`2026-09-02-pace-definition-design.md`](superpowers/specs/2026-09-02-pace-definition-design.md) ·
> 계획: [`2026-09-02-pace-definition.md`](superpowers/plans/2026-09-02-pace-definition.md) ·
> QA 체크리스트: [`2026-09-02-pace-definition-device-checklist.md`](qa/2026-09-02-pace-definition-device-checklist.md)

<details>
<summary>착수 시점 기록 (2026-09-02)</summary>

> 아래 「다음 착수 순서」 1순위 「페이스 표시·계산」 축에서 나왔다(백로그 ④⑤).
> 착수 근거는 2026-08-30 실사용 관찰이다 — 1km 발화 직후 잠금화면을 보니 페이스가 크게 달라
> 있어 오작동으로 읽혔다.
>
> - [x] `pace-definition` — 트래킹 화면과 잠금화면 모두에 현재 페이스와 평균 페이스를 나란히
>   두고 라벨로 구분하며, 현재 페이스 창을 30초에서 줄인다(10초가 출발점, 실기기에서 확정)

- 작업 종류: **작은 기능** — 설계 문서 ○ · 문서 리뷰 ✕ · 계획 문서 ○ · 실기기 QA ○ ·
  최종 브랜치 리뷰는 표에서 ✕이나 실제로 실행했다(위 참고).
...
> **아카이빙 대상이 아니다** — `trace-archive`는 MVP 단위로 도는데 이건 MVP가 아니다.
> 계획 문서는 `docs/superpowers/plans/`에 그대로 둔다.
>
> **결과: Task 4개 중 3개 완료 + 1개 절반.** 백로그 4건을 닫고 2건은 열어뒀다.
> **진행 중 사전 예상이 두 번 뒤집혔다** — ①"호출부는 하나뿐"이 실제로는 셋,
> ②`@MainActor` 건은 "가짜 구현체만 깨진다"던 예상과 정반대로 **진짜 구현체만** 깨졌다.
> 후자는 프로토콜을 하나씩 떼고 각각 빌드하는 방식으로 바로잡았다.
>
> | Task | 결과 |
> |---|---|
> | 1. 죽은 `ContentView` + `cameraStateStore` 주입 구멍 | ✅ 완료 |
> | 2. `RunHistoryViewModel` 위치·이름 | ⚠️ **위치만 이동** — 개명은 새 명명 범주 결정이라 보류(백로그 `open`) |
> | 3. 테스트 전용 코드 출시 빌드 제외 | ✅ 완료 (Release 바이너리 실측 0건) |
> | 4. Domain 프로토콜 `@MainActor` 제거 | ✅ **4개 전부** — `nonisolated`는 0곳, 대신 구현체 3곳 async 멤버에 명시 |
>
...
> 2026-08-03 학습 청크 0에서 드러난 앱 골격의 잔재·구멍 5건을 앱 동작 변경 없이 정리한다.
> **MVP에 편입하지 않고 독립 사이클로 돌리기로 결정(2026-08-04).** 근거: ①진행 중 MVP가
> 없고 ②다음 MVP "코스 연동"과 대상들이 무관하며(앱 골격 잔재) ③`workflow.md`의 새 MVP
...
| 유형 | 파일 | 핵심 내용 |
|------|------|----------|
| 기획 | [킥오프 결정](../history/mvp17/2026-07-21-mvp17-run-history-kickoff-design.md) | 원인 B+C 확정, 기록 탭 신설, RunStats 공유 계산기, 마일스톤 4개 순서·경계 규칙, §8.3 완료 판정 절차 |
| 기획 | [run-idle-polish 설계](../history/mvp17/2026-07-23-run-idle-polish-design.md) | history-tab QA 실패 근거로 대기 화면 재설계(요약 줄 제거 → 목표→시작 흐름) |
| 기획 | [sheet-drag 설계](../history/mvp17/2026-07-26-sheet-drag-design.md) | 리스트 끝 접촉 판정 규칙, `simultaneousGesture` 동시 인식, `ce-doc-review` 6인 반영 |
| 플랜 | [history-tab 플랜](../history/mvp17/2026-07-21-history-tab.md) | Task 다수, RunStats 계산기 + 기록 탭 신설 + 이사 + 대시보드 |
| 플랜 | [run-idle-polish 플랜](../history/mvp17/2026-07-26-run-idle-polish.md) | 대기 화면 재구성 |
| 플랜 | [sheet-drag 플랜](../history/mvp17/2026-07-26-sheet-drag.md) | Task 다수, 시트 리스트 드래그-인계 구현 |
| 플랜 | [lint-cleanup 플랜](../history/mvp17/2026-07-27-lint-cleanup.md) | Task 1~5, 규칙 예외·이름 정리·CodingKeys 고정·줄나눔·문서 종결 |
| QA | [history-tab 실기기 체크리스트](../history/mvp17/2026-07-23-history-tab-device-checklist.md) | GPX 세션 1·2, 다크모드·큰 글자 검증 |
| QA | [run-idle-polish 실기기 체크리스트](../history/mvp17/2026-07-26-run-idle-polish-device-checklist.md) | 목표 설정→시작→저장 흐름 세션 1·2 검증 |
| QA | [sheet-drag 실기기 체크리스트](../history/mvp17/2026-07-26-sheet-drag-device-checklist.md) | 체크포인트 7개, 인계 규칙·대칭성 검증 |
| 회고 | [MVP17 완료 회고](../history/mvp17/260727_mvp17_completion_retro.md) | §8.2 저장 키 오분류 발견, severity 승격 사전 차단, §8.3 조건부 통과 판정 |
