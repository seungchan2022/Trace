...
### MVP17 — 러닝 기록 관리 + 대기 화면 보강   (상태: `history-tab` · `run-idle-polish` 완료 · `sheet-drag` 설계·플랜 확정, 구현 대기)
