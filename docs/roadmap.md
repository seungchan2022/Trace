...
- [ ] **run-detail-polish** — 시작 카운트다운 + 종료 홀드 링 + 발화 문안·속도 정비 + 목표 직접 입력 (경량 사이클: 태스크별 리뷰만)
- [ ] **run-waypoints** — 포인트 찍기: 트래킹 버튼·즉시 발화, 잠금화면 버튼(무세션 가드), additive 스키마 확장, 기록 상세 표시·삭제 (표준 사이클: + 최종 브랜치 리뷰)
