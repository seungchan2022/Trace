...
### 진행 중 · MVP 밖 독립 사이클 `pace-dedup` (작은 기능 — 2026-09-03 정비에서 승격)

> 같은 축에서 나온 나머지 세 건(백로그 ①②③) — 평균 페이스 계산식 복사본과 앱·위젯 포맷터
> 이중 구현을 정리한다. 60분/km 상한의 근거 기록은 포맷터를 손보는 김에 함께 남긴다(같은 줄이다).
>
> 모아 올 **목적지는 `pace-definition`이 이미 만들어 뒀다** — `RunSession`의 활동 시간 기준 평균
> 페이스 계산 프로퍼티다. **착수 조건은 2026-09-03에 풀렸다**(`pace-definition` 완료).
>
> 🔴 **착수 직후 정비에서 작은 기능으로 승격했다(2026-09-03, 사용자 승인).** 대상을 실측하니
> 백로그가 네 곳이라 한 계산식이 **일곱 자리**였고, 그중 둘은 `RunSession`이 없는 자리라 모을 수
> 없으며, 요약 경로를 모으려면 세션에 진입점을 새로 만들고 위젯 타깃 구성을 건드려야 한다.
> 정비 조건 중 「범위가 좁고 명확하다」가 깨졌고, **예외 경로 하나에서는 동작도 달라진다** —
> 위치 권한이 회수돼 끝난 러닝의 요약에서 시간 기준이 바뀌고 평균 페이스가 보이게 된다.
...
> 기록했다. 규칙 개정은 이 사이클을 끝낸 뒤에 한다(한 브랜치는 한 가지 변경이다).
...
| 순위 | 축 | 건수 | 단위 제안 | 조건 |
|---|---|---:|---|---|
| ~~1~~ **2/5 완료** | 페이스 표시·계산 | 5 중 2건 완료(`pace-definition`), 3건 남음(`pace-dedup`) | **MVP 밖 독립 사이클 2개** — 사용자가 새로 할 수 있게 되는 일이 없어 MVP 기준의 「가치 완결」을 충족하지 않는다. 이미 있는 표시의 정정 + 내부 중복 정리다 | 발동 — 2026-08-30 실사용 관찰(발화와 잠금화면의 페이스가 달라 오작동으로 읽혔다) |
