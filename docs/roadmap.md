...
### 진행 중 · MVP 밖 독립 사이클 `pace-definition` (작은 기능, 착수 2026-09-02)
...
> 있어 오작동으로 읽혔다.
>
> - [ ] `pace-definition` — 트래킹 화면과 잠금화면 모두에 현재 페이스와 평균 페이스를 나란히
>   두고 라벨로 구분하며, 현재 페이스 창을 30초에서 줄인다(10초가 출발점, 실기기에서 확정)
>
> ⚠️ **이 사이클의 결정이 아래 `pace-dedup`의 범위를 정한다.** 잠금화면에 평균 페이스를 넘기는
> 방식이 중복 정리 대상을 바꾸기 때문이다. 반대 순서로 하면 중복을 한 곳으로 모은 직후 그
> 자리를 다시 뜯게 된다.
>
...
> 남긴다(같은 줄이다).
>
> 모아 올 **목적지는 `pace-definition`이 먼저 만든다** — `RunSession`의 활동 시간 기준 평균 페이스
> 계산 프로퍼티다. 그래서 이 사이클의 범위는 「네 곳을 그 자리로 옮긴다」로 줄어든다.
>
> 새 동작이 없어 설계 문서 없이 **대상 목록·분류 표**로 시작한다(`workflow.md` 정비 작업 경로).
>
> **`pace-definition`이 끝난 뒤 착수한다.**
...
| 순위 | 축 | 건수 | 단위 제안 | 조건 |
|---|---|---:|---|---|
| ~~1~~ **착수** | 페이스 표시·계산 | 5 (+딸려올 후보 2) | **MVP 밖 독립 사이클 2개** — 사용자가 새로 할 수 있게 되는 일이 없어 MVP 기준의 「가치 완결」을 충족하지 않는다. 이미 있는 표시의 정정 + 내부 중복 정리다 | 발동 — 2026-08-30 실사용 관찰(발화와 잠금화면의 페이스가 달라 오작동으로 읽혔다) |
