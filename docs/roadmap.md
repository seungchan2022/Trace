...
### MVP16 — 러닝/코스 UI 개편   (상태: 진행 중 — 킥오프 완료 2026-07-19)
...
- [x] **run-fullscreen** — 러닝 탭 나이키식 전체화면 재구성. 지도를 대기·트래킹·요약 화면 전부에서 완전히 제거하고, 카운트다운(3-2-1)을 `RunSession.State.countingDown`으로 승격해 탭바 숨김이 카운트다운 진입 시점부터 상태 하나로만 결정되게 만들었다 — tab-restructure에서 갇힘 버그를 막으려고 임시로 넣었던 `RootView`의 강제 탭 전환 우회(커밋 `c7d55fe`)를 제거하고, 탭 전환 창 자체가 생기지 않는 근본 설계로 교체했다(킥오프 중 제안된 아이디어를 그대로 편입). 트래킹 화면은 "거리가 주인공" 위계로 재구성했고, 요약 화면과 기록 목록·상세는 모달 시트 대신 러닝 탭 안 `NavigationStack` push(탭바 계속 노출)로 정리했으며, 기록 상세 지도에는 포인트 구간별 색상 폴리라인 + 구간 표 색 스와치를 추가했다. Task 6개 전부 구현·태스크별 리뷰 통과. 자동 테스트 350개 전체 통과 + 러닝 탭 지도 잔재 없음(grep 스윕 확인) + 시뮬레이터로 대기→카운트다운→취소·기록 목록/상세 이동까지 직접 확인(카운트다운 중 탭 전환 자체가 불가능한 것 포함). 실기기 GPS가 필요한 트래킹 화면 가독성 확인은 사용자 QA로 이월. 플랜: `docs/superpowers/plans/2026-07-20-run-fullscreen.md` · QA 체크리스트: `docs/qa/2026-07-20-run-fullscreen-device-checklist.md`.
