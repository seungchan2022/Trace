...
```bash
git add Trace/Pages/CoursePlannerPage/CoursePlannerPageViewModel.swift TraceTests/CoursePlannerViewModelTests.swift
...
struct MapViewRepresentable: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: MKMapView, context: Context) {
        ...
        } else if context.coordinator.lastSelectedIndex != selectedSegmentIndex {
            uiView.renderer(for: .init())  // no-op guard removed below; see rerender call
        }
...
`updateUIView` 안의 `uiView.renderer(for: .init())` 줄(선택 상태 변경 시 즉시 재적용을 시도하다 남은 자리)은 실제로 불필요한 no-op이므로 **작성하지 말고 지운다** — 바로 아래 `if context.coordinator.lastSelectedIndex != selectedSegmentIndex { ... }` 블록 하나로 충분하다. 최종 코드에는 그 줄을 포함하지 않는다.
...
- **플레이스홀더 스캔**: 없음 — Task 4 Step 1의 `uiView.renderer(for: .init())` 언급은 "쓰지 말라"는 명시적 지시이며 최종 코드 블록에는 포함하지 않음.
