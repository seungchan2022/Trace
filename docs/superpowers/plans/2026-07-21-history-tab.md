...
**QA 보정·수용(2026-07-26):** 실기기 QA 2-1에서 다크 모드의 시스템 검정 배경과 7% 테두리
기준선이 읽기 어려운 것으로 확인됐다. 기록 탭에 `Surface2` 배경을 명시하고 차트 기준선을
`Ink2` 72%로 올렸으며, 사용자 수용으로 이 항목을 닫는다. VoiceOver는 이번 마일스톤의 QA 통과
기준에서 제외한다.
...
- [x] 러닝 종료 → 대기 화면 복귀 시 요약 줄이 갱신된다 (실기기 QA 세션 1 통과)
- [x] 저장 중에는 요약 닫기가 비활성화되고, 저장 성공 뒤 공유 기록이 갱신된다 (실기기 QA 세션 1 통과)
- [x] 차트가 다크모드·Dynamic Type 두 조건에서 동작한다 (2-1 보정 후 사용자 수용)
