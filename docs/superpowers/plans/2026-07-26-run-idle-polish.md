...
- [~] **Step 5: 시뮬레이터로 라이트·다크 모드에서 직접 확인한다.** 러닝 탭에서 자유/거리/시간을 차례로 선택해 입력 영역과 버튼 활성 상태를 확인한다. 통과한 UI 테스트가 첨부한 다크 화면 스크린샷에서 제목·목표 카드·시작 버튼의 순서와 잘림 없음을 확인했다. 큰 글자 강제 주입은 테스트 하니스에 반영되지 않아, Task 3 실기기 QA의 큰 글자 체크포인트로 이관한다.
