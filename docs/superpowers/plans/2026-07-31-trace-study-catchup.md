...
- [ ] **1. 손으로 그은 선이 도로 경로가 되는 법** → `docs/study/1-drawn-path-to-route.html`
      ...
      · **진행 중 — 소개 4절 + 파트 1/7 완료. 다음은 파트 2 「약속을 정한다」.**
      · **브랜치 `docs/study-chunk-1`에서 이어서 한다** (새 브랜치를 만들지 않는다).
      · 파트 구성(만드는 순서): 1 데이터 그릇 → 2 프로토콜 → 3 MapKit 어댑터 → 4 탭 조립 →
      5 다운샘플러 → 6 그리기 조립 → 7 제스처. 노트 맨 아래 "3분 복습"에 능력 선언이 다 있다.
      · **파트 7에 답할 질문이 예약돼 있다**(2026-08-04 사용자 질문): MapKit 기본 제스처 vs
      우리가 얹은 것 · 더블탭 확대 등과 어떻게 안 부딪히게 했나 · 탭 마커는 커스텀인가.
      · 이 세션에서 스킬 자체도 바뀌었다(커밋 `366b089`) — 파트 축이 **만드는 순서**로,
      소개 3종이 필수로, "깨지고 고친 과정"이 필수 항목으로. `SKILL.md`를 다시 읽고 시작할 것.
