...
>   페이스(`5'54"`)가 바이트 단위로 동일했고, 시스템이 자체 렌더링하는 시간(`Text(timerInterval:)`)
>   만 `1:--`→`6:--`로 앞서 나갔다. 표시된 평균 페이스(`5'54"`=354s/km)를 `0.06km`에 대입하면
>   354×0.06≈21초 — 스트림을 끊은 시점과 거의 일치해, 마지막 갱신이 끊긴 그 순간의 값을 그대로
>   물고 있음을 계산으로도 뒷받침한다. **표시 코드(이 Task)의 결함이 아니라 갱신 트리거
