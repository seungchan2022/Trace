...
Task 4 완료:
- 두 스킴(`Trace` 앱, `TraceWidgetsExtension`) 빌드를 각각 따로 돌려 성공 확인 — 구현자 주장에 더해
  최종 브랜치 리뷰어가 독립 재검증함.
...
- [x] 실기기 QA — **조건부 면제**(설계 「실기기 QA」 절의 판단). 체크리스트를 새로 쓰지 않는다.
- [x] 잠금화면 페이스 표시 확인을 **백로그의 실주행 확인 항목으로 올린다.** `pace-definition`이
      ...
      → `docs/backlog.md`의 「pace-definition (2026-09-03) 실기기 QA 피드백」 절, 10초 창 항목
      바로 아래에 추가했다.
- [x] `docs/roadmap.md` — 「진행 중」의 `pace-dedup` 항목을 「완료 · MVP 밖 독립 사이클」로 옮기고
      결과·검증·예상이 빗나간 지점을 남긴다.
- [x] `docs/current-mvp.md` — 현재 상태와 다음에 착수할 수 있는 것을 갱신한다.
- [x] `docs/backlog.md` 3건(평균 페이스 식 복사 · 60분 상한 근거 · 포맷터 이중 구현)을
      ...
      → **실제로는 셋이 아니라 일곱 건을 올렸다.** 이 예고 셋에 더해, 최종 브랜치 리뷰(opus)가
      브랜치 전체를 한 번에 보는 시야에서 넷을 더 찾았다 — `RunSession.streamEnded()`가
      `finish(now:)`로 접을 수 있는 완전 중복이 됨 · `.swiftlint.yml`에 `TraceWidgets` 타깃이
      통째로 빠짐 · 60분/km 경계가 `RunAnnouncementBuilder.spokenPace`에도 상호 참조 없이
      반복됨 · `RunPage+StatsPanelComponent.swift:273`의 GPS-구간 폴백이 도달 불가 코드가 됨.
      개별 Task 리뷰(각자 자기 diff만 봄)에서는 안 보이고 전체 리뷰에서만 드러난 것들이다.
- [x] `docs/workflow-audit.md` §6의 「다음에 관찰할 것」 둘을 판정한다 — §5-9 처방(이 「종료」 절
      체크박스)이 작동했는지, §5-10의 처방 후보 셋이 실제로 필요한 만큼인지.
      → §6-A에 기록: 둘 다 처방을 그대로 유지한다. §5-9는 작동했지만 커밋 전 중간 상태는
      체크박스가 아니라 `git status`의 unstaged diff로 복구됐다는 점이 추가로 드러났다. §5-10의
      승격 뒤 경로(설계→계획→구현)는 과하지 않았다 — 최소 세 지점에서 실제로 값을 냈다.
