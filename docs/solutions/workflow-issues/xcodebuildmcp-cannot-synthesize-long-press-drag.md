...
## 다른 경로: computer-use MCP로 한 손가락 제스처 다섯 개 합성 성공 (2026-08-10 검증 완료)

**한 손가락 제스처 다섯 개가 computer-use로 검증됐다** — 롱프레스-드래그 · 싱글탭 · 더블탭 ·
원핑거 줌 · 끝점 이어 그리기. **시도한 것은 전부 됐다.**

⚠️ **두 손가락 제스처는 미검증이다 — "불가"가 아니다.** `handleDraw:531-539`의
`numberOfTouches > 1` 취소 경로(두 손가락 = 스트로크 취소 + 지도 이동 복귀)와 핀치·회전·피치
(`isPitchEnabled`/`isRotateEnabled`, 337-338행)는 **시도하지 않았다.** computer-use는 커서가
하나라 어려워 보이지만, 시뮬레이터의 Option-드래그 두 손가락 시뮬레이션이 되는지 **확인해
보지 않았다.** 여기서 "구조적으로 불가"라고 적으면 아래 08-07 오판과 똑같은 실수가 된다.
...
|---|---|---|
| 롱프레스-드래그 | 경로선 0.94km + **지도 안 움직임** | 1회 |
| 싱글탭 | 「출발」 마커 생김 | 1회 |
| 더블탭 | 마커 안 늘어남 + **지도 줌인** | 1회 |
| 원핑거 줌 | 마커 안 늘어남 + **줌 변화** | 2회 |
| 끝점 이어 그리기 | 「구간 2」 + 거리 누적(1.06 → 1.49km) | 1회 |
