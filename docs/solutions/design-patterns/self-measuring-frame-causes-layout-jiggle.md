...
**이 문서의 결론은 부분적으로만 맞았다.** 위 수정은 "구간을 추가할수록 시트가 점점 커지는"
증상은 확실히 없앴지만, 사용자가 계속 보고하던 "로딩 중 잠깐 움찔거렸다 돌아온다" 증상은
**이 수정 이후에도 그대로 남아있었다** — 후속 세션(2026-07-13)에서 사용자가 재현을 재확인할
때까지 이 사실이 드러나지 않았다. 실제 근본 원인은 완전히 다른 곳(`sheetHeader`의
`HStack(alignment: .firstTextBaseline)`)에 있었다 — 자세한 내용은
[[firsttextbaseline-alignment-jiggle-with-mixed-child-types]] 참고.

**왜 착각했는가:** 자기측정 순환 패턴은 이 컴포넌트에 실제로 존재했고 실제로 한 가지 버그를
일으키고 있었다("커진다" 증상). 그런데 사용자가 보고한 "움찔거렸다 돌아온다" 증상과 이름이
비슷해서, 같은 원인일 거라고 성급히 결론짓고 검증 없이 문서화했다. 두 증상은 표면적으로
비슷해 보여도 원인이 완전히 달랐다 — "커진다"는 콘텐츠 양에 따라 최종 크기가 달라지는
문제였고, "움찔거렸다 돌아온다"는 로딩 중 일시적으로만 나타나는 베이스라인 정렬 문제였다.

**교훈**: "레이아웃이 움찔거린다"는 보고를 받으면, 이 문서의 체크리스트(자기측정 순환)만
확인하고 끝내지 말 것. 실제로 어떤 하위 뷰의 높이가 바뀌는지 **직접 실측**(임시
`onGeometryChange` + 숨김 진단 `Text` + XCUITest 폴링, `-traceUITesting` 플래그로 게이팅)
해서 확인하기 전에는 "원인을 찾았다"고 단정하지 않는다. 코드 추론만으로 "이 패턴이 있으니
이게 원인이겠다"고 넘겨짚은 게 이번 정정의 근본 원인이다.
