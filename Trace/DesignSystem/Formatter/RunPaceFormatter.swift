...
enum RunPaceFormatter {
    ...
    ///
    /// 주의: 위젯 타깃의 RunLiveActivityWidget.paceText(_:)·averagePaceText(_:) 둘 다에 동일 로직이
    /// 중복되어 있다(위젯 타깃은 이 타입을 볼 수 없어 Target Membership 없이는 재사용 불가).
    /// 여기를 고치면 그쪽 둘 다 같이 고칠 것.
